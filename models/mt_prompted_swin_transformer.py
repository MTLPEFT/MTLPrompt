...
"""
'''
https://github.com/KMnP/vpt/blob/main/src/models/vit_prompt/swin_transformer.py (prompted_swin_transformer.py)
...
from .mt_swin_transformer import (
...
class PromptedSwinTransformer(SwinTransformer):
    def __init__(
        ...
        num_experts=16
        self.mope_0=MoPE(self.embed_dim, self.embed_dim, num_tokens, num_experts,val)
        self.mope_1=MoPE(self.embed_dim*2, self.embed_dim*2, num_tokens, num_experts,val)
        self.mope_2=MoPE(self.embed_dim*4, self.embed_dim*4, num_tokens, num_experts,val)
        self.mope_3=MoPE(self.embed_dim*8, self.embed_dim*8, num_tokens, num_experts,val)
...
class MultimodalRouter(nn.Module):
    def __init__(self, vision_dim, expert_dim, num_experts):
        super(MultimodalRouter, self).__init__()
        self.vision_proj = nn.Linear(vision_dim, expert_dim)
        self.routing_embeddings = nn.Parameter(torch.randn(1,num_experts, expert_dim), requires_grad=False)
        self.temperature = 0.1
        self.num_experts = num_experts
        self._initialize_routing_embeddings()

    def _initialize_routing_embeddings(self):
        with torch.no_grad():
            nn.init.orthogonal_(self.routing_embeddings[0])  #expert 임베딩들이 서로 다른 방향을 가지도록 합니다

    def forward(self, vision_features):
        vision_embedding = self.vision_proj(vision_features)  # Shape: (batch_size, expert_dim)
        B=vision_embedding.shape[0]

        #print(torch.dot(self.routing_embeddings[0][0],self.routing_embeddings[0][15].T))  #내적으로 확인한다.  -2.7940e-08 ==0

        routing_embeddings = self.routing_embeddings.expand(B, -1,-1)  # Shape: (batch_size, num_experts, expert_dim)

        # 샘플된 노이즈 추가
        #noise = torch.randn(vision_embedding.size(0), self.num_experts).to(vision_embedding.device)
        routing_scores = torch.bmm(vision_embedding, routing_embeddings.transpose(1, 2)) / self.temperature #+ noise
        routing_weights = F.softmax(routing_scores, dim=-1)  # Shape: (batch_size, num_experts)

        return routing_weights


class MoPE(nn.Module):
    def __init__(self, vision_dim, expert_dim, prompt_len, num_experts,val):
        super(MoPE, self).__init__()
        self.router = MultimodalRouter(vision_dim, expert_dim, num_experts)
        self.num_experts = num_experts
        self.prompt_len = prompt_len


        # Prompt Experts
        self.prompt_experts = nn.Parameter(torch.randn(num_experts, prompt_len, expert_dim))
    ...
    def forward(self, vision_features):
        ...
        for i, expert in enumerate(self.prompt_experts):
            prompt_embd =expert.expand(B, -1, -1) # self.prompt_dropout()

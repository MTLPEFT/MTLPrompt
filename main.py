...
from models.MTLprompt import mark_only_prompt_as_trainable
...
def main(config):
    ...
    if config.MODEL.MTLPROMPT.ENABLED:
        if config.MODEL.MTLPROMPT.FREEZE_PRETRAINED:
            print("\nMarking Prompt params only as trainable:")
            mark_only_prompt_as_trainable(model.backbone,
                                        bias=config.MODEL.MTLPROMPT.BIAS,
                                        ...
                                        freeze_downsample_reduction= config.TRAIN.FREEZE_DOWNSAMPLE_REDUCTION)
        else:
            print("Marking all layers as trainable")
    print("=============================================================================after mark_only_prompt_as_trainable")
    for name, p in model.named_parameters():
        print(f"{name} : {p.requires_grad}")
    print("=============================================================================after mark_only_prompt_as_trainable")
    if config.MODEL.FREEZE_BACKBONE:
        assert (not config.MODEL.MTLPROMPT.ENABLED)
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, task=None, teacher=None):
    ...
    for idx, batch in enumerate(data_loader):
        ...
        if wandb_available:
            ...
            metrics[f"train/tasks/importance_loss/loss"] = importance_loss.item()
